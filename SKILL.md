...
3. 保持生成边界清晰。
   ...
   - 共享 client、repository、框架依赖配置放入 `AppState` 或服务状态，不散落在 handler 里。
...
5. 显式使用生产默认配置。
   - 新生成 REST/RPC 服务默认使用 `RestServiceConfig` / `RpcServiceConfig` 读取框架通用配置；RPC endpoint、etcd discovery、model database 放入框架配置的 `[rpc_clients]` / `[database]`，不要再放进业务自定义配置。
...
6. 如果 API 调 RPC，把 endpoint 写入 `[rpc_clients.<name>]`，从 `RestServiceConfig::rpc_client_config("<name>")` 构造 channel/client factory 并放到 `AppState`；业务逻辑调用 `state.<rpc>()` 或类型化 wrapper，不在每个方法里手写 interceptor。
7. 运行 cargo 验证。
...
4. 从 `etc/<service>.toml` 通过 `RpcServiceConfig` 读取配置，不要生成自定义 `config.rs` 或把 listen address 写死。
