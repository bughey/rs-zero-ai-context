...
Generated RPC projects should keep `rpc`, `resil` and `observability` enabled. Unary services generated by `rzcli rpc gen` expose `server_layer_stack()`; mount it on tonic `Server::builder().layer(...)` to emit INFO-level `rpc unary observed` logs when observability is enabled.
