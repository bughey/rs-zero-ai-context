...
4. RPC server 侧优先挂 `RpcServerLayerStack`，让外层 Tower layer 读取 metadata；trait method 内可以直接 `request.into_inner()`。
5. 如果是未挂 `RpcServerLayerStack` 的旧项目或手写高级路径，再使用 `request.into_parts()`、`RpcResilienceLayer::run_unary_with_metadata(...)` 或 `observe_rpc_unary_with_metadata(...)`。
6. 确认生成项目依赖的 `rs-zero` 版本包含 Tower-first RPC server layer；旧版本生成代码即使客户端带了 metadata，server 侧也可能记录为空。
